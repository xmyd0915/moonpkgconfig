...
已实现：变量与字段解析、注释和续行、顺序变量展开、`pcfiledir` 输入、来源位置、错误恢复、编译/链接参数分词以及 `Requires` 依赖表达式解析。
...
`split_flags` 返回参数数组，不启动 shell；`parse_requirements` 返回包名、比较运算符和版本文本，此阶段不会判断依赖是否满足。
