...
| `moon test --target all --deny-warn` | wasm 37/37，wasm-gc 37/37，JS 37/37，native 37/37 |
| `moon run cmd/demo` | 成功，输出六项字段及来源，无解析诊断 |

37 个逻辑测试在四个目标各运行一次，不将它们宣传为 148 个独立测试。覆盖顺序变量展开、未定义/自引用、字面美元符、命名空间与大小写、重复/保留定义、BOM/CRLF/注释/续行/Unicode、包元数据校验、直接与传递依赖检查、私有依赖、循环诊断、参数聚合与来源、参数引号/转义、版本分段比较、六种约束运算符及错误恢复、展开大小上限。
